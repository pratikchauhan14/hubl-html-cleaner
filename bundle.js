(() => {
  ...
  var require_xregexp = __commonJS({
    "node_modules/xregexp/lib/xregexp.js"(exports, module) {
      ...
      var _default = exports["default"] = XRegExp2;
  ...
  var require_build = __commonJS({
    "node_modules/xregexp/lib/addons/build.js"(exports, module) {
      ...
      var _indexOf = _interopRequireDefault(require_index_of4());
      var _concat = _interopRequireDefault(require_concat4());
      var _default = exports["default"] = function _default2(XRegExp2) {
        var REGEX_DATA = "xregexp";
  ...
  var Formatter = class {
    ...
    splitBlocksHTML(content) {
      ...
      function isInsideComment(pos2) {
        return commentRanges.some((r) => pos2 >= r.start && pos2 < r.end);
      }
      let blocks = [];
      let index = 0;
      let pos = 0;
        ...
        if (start > pos) {
          ...
          while ((match = tagRegex.exec(region)) !== null) {
            ...
            if (tagStart > lastIndex)
              value += region.slice(lastIndex, tagStart);
            value += tag;
    ...
    splitBlocksHUBL(blocksHTML) {
      ...
      for (const block of blocksHTML) {
        let isInsideHtmlTag = function(pos, str) {
          let inside = false;
          for (let i = 0; i < pos; i++) {
            if (str[i] === "<")
              inside = true;
          ...
          return false;
        }, getLogicSubtype = function(value2) {
          let inner = value2.replace(/^\{%\s*|\s*%\}$/g, "").trim();
          ...
          if (startKeywords.includes(keyword))
            return "start";
          if (endKeywords.includes(keyword))
            return "end";
          return "start";
        };
        if (block.type !== "html") {
        ...
        for (let i = 0; i < matches.length; i++) {
          const m2 = matches[i];
          if (m2.start > lastPos) {
            const htmlPart = value.slice(lastPos, m2.start).trim();
    ...
    tabIndent(blocks) {
      ...
      for (let i = 0; i < newBlocks.length - 2; i++) {
        if (newBlocks[i].subtype === "start" && newBlocks[i + 1].subtype === "print" && newBlocks[i + 2].subtype === "end") {
